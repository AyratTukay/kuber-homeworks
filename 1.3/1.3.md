...
Пишу манифест Deployment приложения, состоящего из двух контейнеров — nginx и multitool.  
[Ссылка на манифест Deployment](http:erns/create)  
Запускаю Deployment:  
...
Увеличиваю количество реплик работающего приложения до 2:  
...
Создаю Service, который обеспечит доступ до реплик приложений.  
[Ссылка на манифест Service](http:erns/create)  
Применяю манифест:  
...
[Ссылка на манифест multitool](http:erns/create)  
Применяю манифест:  
...
Убеждаюсь с помощью `curl`, что из пода есть доступ до приложений.  
...
[Ссылка на манифест nginx-deploy](http:erns/create)  
Применяю манифест и проверяю его запуск:  

  ![Скрин kubectl apply -f nginx-deploy.yaml](img/img12.png)  

Вижу что nginx-deploy-6ff99c4cf5-ms2sk не запущен, находится на стадий Init:0/1. Требуется создание сервиса для его запуска. Создаю и запускаю сервис nginx-svc.  
[Ссылка на сервис nginx-svc](http:erns/create)  
